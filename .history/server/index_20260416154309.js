...
Si l'utilisateur dit "coucou", répond naturellement.
  ...
  // 🔥 gestion spécifique quota (429)
  if (error?.status === 429) {
    return res.status(200).json({
      reply:
        "⏳ Je suis désolé, j’ai atteint ma limite temporaire. Réessaie dans quelques instants 😊",
    });
  }
  ...
  return res.status(200).json({
    reply:
      "Je suis désolé 😢 (mode limité activé) — je ne peux pas répondre pour le moment. Essaie dans quelques instants.",
  });
}
});
