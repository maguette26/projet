...
const Ressources = () => {
  ...
  const renderResourceContent = (f) => {
    ...
    // Si la ressource est une des ressources gratuites spéciales avec une page dédiée, on affiche un lien
    switch (nom.toLowerCase()) {
      case 'mini defi gratuite':
        return (
          <Link to="/mini-defi-gratuite" className="text-indigo-600 hover:underline mt-2 block font-semibold">
            ▶️ Accéder au Mini Défi Gratuit
          </Link>
        );
      case 'liste de controle bien etre':
        return (
          <Link to="/liste-controle-bien-etre" className="text-indigo-600 hover:underline mt-2 block font-semibold">
            ▶️ Accéder à la Liste de Contrôle Bien-Être
          </Link>
        );
      case 'mini defi decouverte':
        return (
          <Link to="/mini-defi-decouverte" className="text-indigo-600 hover:underline mt-2 block font-semibold">
            ▶️ Accéder au Mini Défi Découverte
          </Link>
        );
      case 'guide : fixateur des limites saines':
        return (
          <Link to="/guide-fixateur-limites" className="text-indigo-600 hover:underline mt-2 block font-semibold">
            ▶️ Accéder au Guide Fixateur des Limites Saines
          </Link>
        );
      case 'auto-evaluation basique':
        return (
          <Link to="/auto-evaluation-basique" className="text-indigo-600 hover:underline mt-2 block font-semibold">
            ▶️ Accéder à l'Auto-Évaluation Basique
          </Link>
        );
      default:
        // Sinon, rendu classique selon le type
        switch (type) {
          case 'citation':
