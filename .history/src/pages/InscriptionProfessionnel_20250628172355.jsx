...
const InputField = ({ id, label, type = 'text', autoComplete, value, onChange, error, placeholder }) => (
  <div className="flex flex-col">
    ...
    <input
      ...
      className={`border rounded-md px-4 py-3 text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 transition
        ${error ? 'border-red-500 focus:ring-red-400' : 'border-gray-300 focus:ring-indigo-400'}`}
    />
    {error && <p className="text-red-600 text-xs mt-1 italic font-semibold">{error}</p>}
  </div>
);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, val]) => {
        if (val !== null) {
          if (key === 'justificatif') {
            formPayload.append('document', val); // doit correspondre au @RequestParam("document") du backend
          } else {
      ...
      setSuccess("Votre inscription a bien été prise en compte. Veuillez attendre la validation de votre compte par l'administrateur.");
      // Pas de redirection automatique ici, laisse le temps à l'utilisateur de lire le message
    } catch (err) {
      console.error("Erreur d'inscription:", err.response?.data || err.message);
      const message = err.response?.data?.message || err.response?.data;

      if (typeof message === 'string' && message.includes('Duplicate entry') && message.includes('unique_email')) {
    ...
    <Layout>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-white px-6 py-20">
        <div className="w-full max-w-xl bg-white rounded-3xl shadow-lg p-10">
          <h2 className="text-3xl font-semibold text-indigo-700 mb-6 text-center tracking-wide">
          ...
          <form onSubmit={handleSubmit} className="space-y-7" encType="multipart/form-data">
            {errors.general && (
              <div className="bg-red-50 border border-red-400 text-red-700 px-5 py-3 rounded-md text-center text-sm font-semibold mb-6">
                {errors.general}
              </div>
            )}
            {success && (
              <div className="bg-green-50 border border-green-400 text-green-700 px-5 py-3 rounded-md text-center text-sm font-semibold mb-6">
                {success}
              </div>
            )}
            ...
            <div className="flex flex-col">
              ...
              />
              {errors.justificatif && (
                <p className="text-red-600 text-xs mt-1 italic font-semibold">{errors.justificatif}</p>
              )}
            </div>
            ...
            <div className="flex justify-center">
              <button
                type="submit"
                className="w-48 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-base rounded-2xl shadow-md transition duration-300 ease-in-out focus:outline-none focus:ring-4 focus:ring-indigo-400"
              >
                S'inscrire
              </button>
            </div>
