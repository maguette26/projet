...
import { CalendarDays, UserCheck, Smile, User, XCircle } from 'lucide-react';
...
const TableauUtilisateur = () => {
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    setSidebarReduced(true); // Réduction automatique du navbar
  };
  ...
  return (
    <Layout>
      <div className="flex min-h-screen bg-gray-50">
        ...
        >
          <div className="flex items-center gap-4 p-4 border-b border-blue-500 transition-all">
          ...
          <div className="flex-grow space-y-1 mt-4 px-2">
            <NavItem icon={<UserCheck size={20} />} label="Réservations" active={activeTab === 'reservations'} onClick={() => handleTabChange('reservations')} reduced={sidebarReduced} />
            <NavItem icon={<CalendarDays size={20} />} label="Consultations" active={activeTab === 'consultations'} onClick={() => handleTabChange('consultations')} reduced={sidebarReduced} />
            <NavItem icon={<Smile size={20} />} label="Humeur" active={activeTab === 'humeur'} onClick={() => handleTabChange('humeur')} reduced={sidebarReduced} />
            <NavItem icon={<User size={20} />} label="Profil" active={activeTab === 'profil'} onClick={() => handleTabChange('profil')} reduced={sidebarReduced} />
          </div>
        </nav>
