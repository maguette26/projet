...
const InputField = ({ id, label, type = 'text', autoComplete, value, onChange, error, placeholder }) => (
  <div className="flex flex-col">
    <label htmlFor={id} className="mb-2 text-gray-800 font-semibold">{label}</label>
    <input
      id={id}
      name={id}
      type={type}
      ...
      className={`border rounded-xl px-5 py-3 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 transition
        ${error ? 'border-red-500 focus:ring-red-400' : 'border-gray-300 focus:ring-indigo-400'}`}
    />
    {error && <p className="text-red-600 mt-1 italic font-medium">{error}</p>}
  </div>
);
...
const InscriptionProfessionnel = () => {
  ...
  return (
    <Layout>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-white px-6 py-16 text-sm">
        <div className="w-full max-w-xl bg-white rounded-3xl shadow-2xl p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-8" encType="multipart/form-data">
            {errors.general && (
              <div className="bg-red-50 border border-red-400 text-red-700 px-5 py-3 rounded-md text-center font-semibold mb-6">
                {errors.general}
              </div>
            )}
            {success && (
              <div className="bg-green-50 border border-green-400 text-green-700 px-5 py-3 rounded-md text-center font-semibold mb-6">
                {success}
              </div>
            )}
            ...
            <div className="flex flex-col">
              <label htmlFor="justificatif" className="mb-2 text-gray-800 font-semibold">
              ...
              />
              {errors.justificatif && (
                <p className="text-red-600 mt-1 italic font-medium">{errors.justificatif}</p>
              )}
            </div>
