...
const Ressources = () => {
  ...
  const renderResourceContent = (f) => {
    const { type, description, lienFichier, premium, demoText } = f;

    if (premium && !isUserPremium) {
      const previewText = demoText || description?.slice(0, 100) + '...';
      ...
      return (
        <div className="relative mt-3 bg-yellow-50 rounded p-4 overflow-hidden">
          ...
          <div className="opacity-30 pointer-events-none italic text-gray-700">
            {type === 'citation' ? (
