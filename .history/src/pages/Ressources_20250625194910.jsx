...
const Ressources = () => {
  ...
  // Redirection si la catégorie sélectionnée contient que des ressources premium et l'utilisateur n'est pas premium
  useEffect(() => {
  ...
  const renderResourceContent = (f) => {
    ...
    if (premium && !isUserPremium) {
      return (
        <div className="mt-4 flex flex-col gap-3">
          <p className="text-sm text-gray-600">🔐 Cette ressource est réservée aux membres Premium.</p>
          <button
            onClick={() => navigate('/devenir-premium')}
            className="inline-flex items-center justify-center gap-2 rounded-md bg-indigo-600 px-4 py-2 text-white font-semibold shadow hover:bg-indigo-700 transition"
            title="Accéder à la page Devenir Premium"
          >
            🔒 Devenir Premium
          </button>
        </div>
    ...
    if (resourceLinks[normalizedNom]) {
      return (
        <Link
          ...
          className="inline-flex items-center gap-2 mt-4 px-5 py-3 bg-indigo-600 text-white rounded-md font-semibold shadow hover:bg-indigo-700 transition"
          title={`Accéder à la ressource : ${nom}`}
        >
    ...
    switch (type.toLowerCase()) {
      case 'citation':
        return <blockquote className="mt-4 italic text-gray-700 border-l-4 border-indigo-400 pl-4">💬 "{description}"</blockquote>;
      ...
      case 'video':
        if (lienFichier && (lienFichier.includes('youtube.com') || lienFichier.includes('youtu.be'))) {
          ...
          return (
            <div className="mt-4 aspect-w-16 aspect-h-9 rounded-lg overflow-hidden shadow-lg">
              <iframe
        ...
        return (
          <a
            href={lienFichier}
            className="inline-block mt-4 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition font-semibold"
            target="_blank"
          ...
          <div className="mt-4 text-gray-800">
            ...
            {lienFichier && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block mt-2 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition font-semibold"
              ...
              <section className="mb-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {gratuits.map(f => (
                    <motion.div
                      key={f.id}
                      className="bg-white rounded-lg shadow-lg p-6 border border-gray-200 cursor-pointer flex flex-col justify-between"
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {premiums.map(f => (
                    <motion.div
                      key={f.id}
                      className="bg-yellow-50 rounded-lg shadow-lg border border-yellow-300 p-6 cursor-pointer flex flex-col justify-between"
                      variants={cardVariants}
                      initial="hidden"
                      animate="visible"
                      whileHover="hover"
