...
const Ressources = () => {
  ...
  const renderResourceContent = (f) => {
    ...
    if (premium && !isUserPremium) {
      return (
        <div className="relative mt-3 bg-yellow-50 rounded p-4 overflow-hidden">
          <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex flex-col justify-center items-center rounded">
            <p className="text-yellow-800 font-medium mb-3">🔒 Cette ressource est réservée aux membres Premium</p>
            <button
              onClick={() => navigate('/devenir-premium')}
              className="bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-semibold px-4 py-2 rounded shadow"
            >
              Débloquer avec Premium
            </button>
          </div>
          <div className="opacity-30 pointer-events-none">
            <p className="italic truncate">{description.slice(0, 100)}...</p>
          </div>
        </div>
      );
    }
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {premiums.map(f => (
                    <motion.div
                      key={f.id}
                      className="bg-yellow-50 rounded-lg shadow border border-yellow-300 p-5 cursor-pointer relative"
                      variants={cardVariants}
                      initial="hidden"
                      animate="visible"
                      whileHover="hover"
