...
const Ressources = () => {
  ...
  // Pages dédiées pour les ressources spéciales
  const pagesSpecifiques = {
    'mini defi gratuite': '/mini-defi-gratuite',
    'liste de controle bien etre': '/liste-controle-bien-etre',
    'mini defi decouverte': '/mini-defi-decouverte',
    'guide : fixateur des limites saines': '/guide-fixateur-limites',
  ...
  const renderResourceContent = (f) => {
    ...
    // Pour les ressources spéciales, on affiche juste un résumé avec un lien
    if (pagesSpecifiques[nom.toLowerCase()]) {
      return (
        <Link
          to={pagesSpecifiques[nom.toLowerCase()]}
          className="text-indigo-600 hover:underline mt-2 block font-semibold"
        >
          ▶️ Accéder à {nom}
        </Link>
      );
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {gratuits.map(f => (
                    <motion.div
                      ...
                      onClick={() => {
                        if (pagesSpecifiques[f.nom.toLowerCase()]) {
                          navigate(pagesSpecifiques[f.nom.toLowerCase()]);
                        }
                      }}
                    >
