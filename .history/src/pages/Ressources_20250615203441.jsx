...
const Ressources = () => {
  ...
  // Regroupe par catégorie puis par gratuit/premium
  const filteredAndGroupedFonctionnalites = useMemo(() => {
    ...
    filtered.forEach(f => {
      ...
      if (!groups[typeKey]) {
        groups[typeKey] = { gratuit: [], premium: [] };
      }
      if (f.premium) {
        groups[typeKey].premium.push(f);
      } else {
        groups[typeKey].gratuit.push(f);
      }
    });
        ...
        {loading ? (
          ...
          <div className="space-y-12">
            {Object.keys(filteredAndGroupedFonctionnalites).map(typeKey => {
              const group = filteredAndGroupedFonctionnalites[typeKey];
              return (
                <section key={typeKey}>
                  <h2 className="text-2xl font-bold text-gray-800 mb-4">{getCategoryTitle(typeKey)}</h2>

                  {/* Ressources gratuites */}
                  {group.gratuit.length > 0 && (
                    <>
                      <h3 className="text-xl font-semibold text-green-700 mb-2">Ressources Gratuites</h3>
                  ...
                  {group.premium.length > 0 && (
                    <>
                      <h3 className="text-xl font-semibold text-yellow-700 mb-2">Ressources Premium</h3>
