        ...
        return (
          <div className="p-8 bg-white rounded-lg shadow-md max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold text-gray-900 mb-4 flex items-center">
              ...
              <Smile className="w-7 h-7 text-yellow-400 ml-3" />
            </h2>
            <p className="text-gray-700 text-lg leading-relaxed max-w-prose mb-6">
              Bienvenue sur votre tableau de bord. Ici, vous pouvez gérer les utilisateurs, les professionnels, modérer les discussions, et configurer les fonctionnalités de la plateforme.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-xl">
              {menuItems
                .filter(item => item.key !== 'tableauDeBord') // On ne montre pas la page d'accueil ici
                .map(({ key, label, icon }) => (
                  <button
                    key={key}
                    onClick={() => setActiveSection(key)}
                    className="flex items-center gap-3 p-4 border border-gray-300 rounded-md hover:bg-indigo-50 transition"
                  >
                    {icon}
        ...
        <div className="p-4 border-t border-blue-700 mt-auto">
          ...
          >
            ...
            >
              ...
              <path strokeLinecap="round" strokeLinejoin="round" d="M7 16v-1a4 4 0 014-4h3" />
            </svg>
            Déconnexion
          </button>
        </div>
      </aside>
      <main className="flex-grow overflow-y-auto p-8">{renderContent()}</main>
    </div>
  );
};
