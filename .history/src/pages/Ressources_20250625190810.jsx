...
useEffect(() => {
  console.log("Fonctionnalités reçues :", fonctionnalites.map(f => f.nom));
}, [fonctionnalites]);
  useEffect(() => {
  ...
  const renderResourceContent = (f) => {
    ...
    // Normalisation du nom : minuscules, sans accents, sans tirets ni deux-points, trim
    ...
    // Mapping des noms normalisés vers les URLs internes
    const resourceLinks = {
    ...
    // Si ressource premium et utilisateur non premium : bouton devenir premium
    ...
    // Si lien interne spécial existant pour cette ressource
    if (resourceLinks[normalizedNom]) {
      return (
        <Link to={resourceLinks[normalizedNom]} className="text-indigo-600 hover:underline mt-2 block font-semibold">
          ▶️ Accéder à la ressource
        </Link>
      );
    }

    // Rendu classique selon le type
