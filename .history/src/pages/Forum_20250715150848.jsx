...
import { AlertTriangle, X, Check, ChevronLeft } from 'lucide-react';
import {
  getForumSujets,
  creerForumSujet,
  getProfil,
...
const Forum = () => {
  ...
  const [notConnectedMessage, setNotConnectedMessage] = useState(false);
  ...
  useEffect(() => {
    const fetchAuthStatus = async () => {
      try {
        ...
        if (profil?.authenticated) {
          setIsAuthenticated(true);
          setCurrentUserEmail(profil.email);
          setCurrentUserRole(profil.role);
        } else {
          setNotConnectedMessage(true);
          setTimeout(() => {
  ...
  // 👉 Message non connecté avec animation
      ...
      <Layout>
        ...
        >
          ...
          <h2 className="text-xl font-semibold text-red-700 text-center">
            ⚠️ Vous devez être connecté pour accéder au forum.
          </h2>
