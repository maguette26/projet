...
import { Coffee } from 'lucide-react'; // Exemple d'icône lucide (emoji café sympa)
...
const TableauAdmin = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      <aside
        className={`bg-blue-800 text-white flex flex-col h-full shadow-lg transition-width duration-300 ${
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-blue-700">
          <div className="flex items-center gap-2">
            ...
            {!sidebarCollapsed && (
              <>
                <span className="text-blue-200 text-lg font-bold select-none">Psy</span>
                <span className="text-white text-lg font-bold select-none">Connect</span>
        ...
        <nav className="flex-grow mt-4 overflow-y-auto">
          <ul>
            {menuItems.map(({ key, label, icon }) => (
              <li key={key} className="mb-2" title={sidebarCollapsed ? label : ''}>
                <button
                  onClick={() => setActiveSection(key)}
                  className={`w-full flex items-center px-4 py-3 transition-colors duration-200 rounded-md
                    ${
                      activeSection === key
                ...
                >
                  {React.cloneElement(icon, {
                    className: `${icon.props.className} ${sidebarCollapsed ? 'mr-0' : 'mr-3'}`,
                  })}
