...
const InputField = ({ id, label, type = 'text', autoComplete, value, onChange, error, placeholder }) => (
  <div className="flex flex-col">
    ...
    <input
      ...
      className={`border rounded-xl px-3 py-2 text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:ring-3 transition
...
const InscriptionProfessionnel = () => {
  ...
  return (
    <Layout>
      <div
        className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-white px-6 py-12"
        style={{ fontSize: '0.75rem' }} // taille générale réduite
      >
        <div className="w-full max-w-lg bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-extrabold text-indigo-700 mb-5 text-center tracking-wide">
          ...
          <form onSubmit={handleSubmit} className="space-y-6" encType="multipart/form-data">
            {errors.general && (
              <div className="bg-red-50 border border-red-400 text-red-700 px-4 py-2 rounded-md text-center font-semibold mb-4">
                {errors.general}
              </div>
            )}
            {success && (
              <div className="bg-green-50 border border-green-400 text-green-700 px-4 py-2 rounded-md text-center font-semibold mb-4">
                {success}
              </div>
            )}
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                className={`border rounded-xl px-3 py-2 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-3 transition
                  ${errors.justificatif ? 'border-red-500 focus:ring-red-300' : 'border-gray-300 focus:ring-indigo-300'}`}
              />
              {errors.justificatif && (
            ...
            <button
              type="submit"
              className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-xl shadow-md transition duration-300 ease-in-out focus:outline-none focus:ring-3 focus:ring-indigo-400"
            >
              S'inscrire
            </button>
          </form>
