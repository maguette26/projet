...
const InscriptionProfessionnel = () => {
  ...
  const validatePassword = (password) => {
    ...
    if (!/[A-Z]/.test(password)) errors.push("Une lettre majuscule est requise.");
    if (!/[a-z]/.test(password)) errors.push("Une lettre minuscule est requise.");
  ...
  const validateForm = () => {
    ...
    if (!formData.telephone.trim()) {
      ...
      newErrors.telephone = "Le format du numéro est invalide.";
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formPayload.append('motDePasse', formData.motDePasse);
      formPayload.append('document', formData.justificatif);

      await api.post('/professionnels/inscription', formPayload, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });

      setSuccess("Inscription envoyée avec succès. Veuillez attendre la validation de votre compte par l'administrateur.");
    } catch (err) {
    ...
    <Layout>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-white px-6 py-12">
        <div className="w-full max-w-lg bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-semibold text-indigo-700 mb-4 text-center">Inscription Professionnel</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6" encType="multipart/form-data">
            ...
            <InputField id="specialite" label="Spécialité" value={formData.specialite} onChange={handleChange} error={errors.specialite} placeholder="Ex: Psychologue, Psychiatre..." />
            ...
            <div className="flex flex-col">
              ...
              />
              {errors.justificatif && <p className="text-red-600 text-xs mt-1 italic font-medium">{errors.justificatif}</p>}
            </div>
