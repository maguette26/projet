...
const DevenirPremium = () => {
  ...
  return (
    <Layout>
      <main className="flex flex-col items-center justify-start min-h-screen pt-0.5 bg-gradient-to-tr from-indigo-50 via-white to-indigo-50">
        <motion.div
          ...
          {Object.entries(plans).map(([key, plan]) => (
            <div key={key} className="mb-0.5 w-full px-1 border-b border-indigo-100 last:border-none">
              <p className="text-[6px] font-extrabold text-gray-900 leading-none tracking-tight select-none">
                {plan.label} : {plan.price.toFixed(2)} € / <span className="text-[4.5px] text-gray-500">{plan.duration}</span>
              </p>
