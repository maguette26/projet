...
const ListeProfessionnels = () => {
  ...
  const fetchDisponibilites = async (proId) => {
    ...
    console.log("📡 Chargement des disponibilités pour proId =", id, "type =", typeof id);
    ...
    try {
      ...
      const res = await axios.get(`/api/disponibilites/${id}`, { withCredentials: true });
      const pro = professionnels.find(p => p.id === id) || null;
      setSelectedPro(pro);
...
const reserverCreneau = async (dispo, heureConsultation) => {
  ...
  try {
    ...
    if (selectedPro) fetchDisponibilites(selectedPro.id); // rafraîchir les créneaux
  } catch (error) {
