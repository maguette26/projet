...
const fetchDisponibilites = async (proId) => {
  if (!proId || isNaN(proId)) {
    console.error("proId invalide :", proId);
    toast.error("Impossible de charger les disponibilités (ID invalide).");
    return;
  }
  ...
  try {
    ...
    console.error("Erreur lors du chargement des disponibilités :", err.response?.data || err.message);
