...
const formatSpecialite = (spec) => {
  const cleaned = spec.trim().toLowerCase();
  if (cleaned === 'psychologie') return 'Psychologue';
  if (cleaned === 'psychiatrie') return 'Psychiatre';
  return spec.charAt(0).toUpperCase() + spec.slice(1); // Fallback
};
  ...
  const fetchProfessionnels = async () => {
    try {
      ...
      const specs = Array.from(
        new Set(res.data.map(p => formatSpecialite(p.specialite)))
      ).sort();
...
const professionnelsFiltres = selectedSpecialite === 'all'
  ? professionnels
  : professionnels.filter(pro => formatSpecialite(pro.specialite) === selectedSpecialite);
