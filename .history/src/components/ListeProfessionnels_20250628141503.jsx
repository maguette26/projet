...
const ListeProfessionnels = () => {
  ...
  const BoutonPrimaire = ({ onClick, children }) => (
    <button
      onClick={onClick}
      className="inline-flex items-center justify-center gap-3 bg-indigo-700 hover:bg-indigo-800 text-white font-semibold rounded-full px-10 py-3 shadow-lg hover:shadow-indigo-600 transition-all duration-300 active:scale-95"
    >
      {children}
    </button>
  );
  ...
  return (
    <>
      ...
      >
        {/* Sections animées + Modals + Paiement ici */}
      </main>
      <PiedPage />
    </>
  );
