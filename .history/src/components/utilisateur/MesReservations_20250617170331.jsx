...
const MesReservations = () => {
  ...
  // Filtrage combiné : cherche dans professionnelNom OU dans statut (insensible à la casse)
  const filteredReservations = reservations.filter(res => {
    const term = searchTerm.toLowerCase();
    const nomMatch = res.professionnelNom.toLowerCase().includes(term);
    const statutMatch = res.statut.toLowerCase().includes(term);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-xl mt-10">
      ...
      <input
        type="search"
        placeholder="🔍 Rechercher par professionnel ou statut (ex : EN_ATTENTE)"
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
        className="w-full max-w-md mx-auto mb-8 px-3 py-2 text-sm border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500 transition block"
        aria-label="Recherche par professionnel ou statut"
      />
