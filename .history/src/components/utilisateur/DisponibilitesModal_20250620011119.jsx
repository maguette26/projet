            ...
            <ul className="space-y-4">
              {disponibilites.map((dispo, idx) => {
                ...
                const totalSlots = Math.floor(
                  (new Date(`${dispo.date}T${dispo.heureFin}`) - new Date(`${dispo.date}T${dispo.heureDebut}`)) / (45 * 60 * 1000)
                );
                const isFull = sousCreneaux.length === 0 && reservationsActives.length >= totalSlots;
