  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (error) {
        ...
        onSuccess('Paiement Stripe traité (en cours ou réussi)');
      }
...
const PaymentForm = ({ reservationId, onClose }) => {
  ...
  const createPayment = async () => {
    try {
      ...
      console.error("❌ Erreur Axios création paiement :", error);
      throw error;
    }
  };
      ...
      createPayment()
        .then((data) => {
          if (data.clientSecret) {
            ...
            setMessage({ type: 'error', text: "ClientSecret non reçu du serveur." });
          }
        })
        .catch(() => {
  ...
  const handlePayPalPayment = async () => {
    ...
    try {
      ...
      if (data.approvalUrl) {
        ...
        setMessage({ type: 'error', text: "URL d'approbation PayPal manquante." });
      }
    } catch {
