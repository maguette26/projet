...
const MesConsultations = () => {
  ...
  const confirmerPaiement = async (id) => {
    try {
      await axios.put(`/api/consultations/${id}/payer`, {}, {
        withCredentials: true,
      });
      toast.success('Paiement effectué. Statut mis à jour !');
      fetchConsultations(); // Rechargement des données
    } catch (error) {
          ...
          {filteredConsultations.map((c) => (
            ...
            >
              <div className="space-y-2 text-sm md:text-base max-w-xl">
                ...
                {c.statut === 'EN_ATTENTE' && (
                  <button
                    onClick={() => confirmerPaiement(c.id)}
                    className="mt-2 inline-block bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700 transition"
                  >
                    💳 Payer et confirmer
                  </button>
                )}
