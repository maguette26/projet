...
const SuiviHumeur = ({ currentUser }) => {
  ...
  const [messageFlash, setMessageFlash] = useState(null); // {type: 'success'|'error', text: string} ou null
  ...
  // Fonction pour afficher un message flash succès ou erreur qui disparaît après quelques secondes
  const setFlashMessage = (type, text, duration = 3000) => {
    setMessageFlash({ type, text });
    setTimeout(() => {
      setMessageFlash(null);
    }, duration);
  };
  ...
  const handleEnregistrer = async (e) => {
    ...
    try {
      ...
      setSelectedMood('');
      setNotes('');
      setFlashMessage('success', "Humeur ajoutée avec succès !");
    } catch (err) {
      console.error(err);
  ...
  const handleModifier = async () => {
    ...
    try {
      ...
      setFlashMessage('success', "Votre journal intime a été mis à jour avec succès !");
    } catch (err) {
      console.error(err);
  ...
  const handleSupprimer = async () => {
    ...
    try {
      ...
      setModeEdition(false);
      setEditionNotes('');
      setFlashMessage('success', "Humeur du jour supprimée.");
    } catch (err) {
      console.error(err);
    ...
    <div className="max-w-3xl mx-auto p-8 bg-gradient-to-tr from-indigo-100 via-indigo-50 to-white rounded-3xl shadow-lg">
      ...
      {humeurDuJour && (
        <section className="mb-12 bg-white p-8 rounded-2xl shadow-md relative">
          ...
          {!modeEdition && (
            <p className="whitespace-pre-wrap text-indigo-800 font-semibold">{humeurDuJour.noteJournal || "Aucune note pour aujourd'hui."}</p>
          )}
      ...
      {humeurs.length > 0 && (
        <section className="mb-12 bg-white p-8 rounded-2xl shadow-md">
          <h3 className="text-2xl font-semibold mb-6 text-indigo-700 border-b border-indigo-300 pb-3">Historique des humeurs</h3>
          <ul className="space-y-4 max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-400 scrollbar-track-indigo-100">
            {humeurs.map((humeur) => (
              <li
                key={humeur.id}
                className={`p-4 rounded-xl border border-indigo-300 flex items-center justify-between
                  ${humeurDuJour && humeur.id === humeurDuJour.id ? "bg-indigo-200" : "bg-indigo-50"}`}
              >
                <div className="flex items-center gap-4">
                  {moodsList.find(m => m.label === humeur.etat)?.Icon && React.createElement(
                    moodsList.find(m => m.label === humeur.etat).Icon,
                    { size: 28, className: moodsList.find(m => m.label === humeur.etat).color }
                  )}
                  <div>
                    <p className="font-semibold text-indigo-700">{humeur.etat}</p>
                    <p className="text-sm text-indigo-600">{new Date(humeur.date).toLocaleDateString('fr-FR')}</p>
                  </div>
                </div>
                {humeur.noteJournal && (
                  <p className="text-indigo-800 font-medium max-w-xs truncate" title={humeur.noteJournal}>
                    {humeur.noteJournal}
                  </p>
                )}
              </li>
