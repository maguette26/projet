...
const SuiviHumeur = ({ currentUser }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 bg-gradient-to-tr from-indigo-100 via-indigo-50 to-white rounded-3xl shadow-lg">
      ...
      {/* Formulaire d'ajout repensé en ligne, comme historique */}
      {!humeurDuJour && (
        <section className="mb-12">
          <h3 className="text-2xl font-semibold mb-4 text-indigo-700 border-b border-indigo-300 pb-2">
            Ajouter votre humeur du jour
          </h3>
          <form onSubmit={handleEnregistrer} className="flex items-center gap-4 flex-wrap">
            {/* Date fixe */}
            <span className="w-24 font-semibold text-indigo-700">{new Date().toLocaleDateString('fr-FR')}</span>

            {/* Sélecteur humeur compact */}
            <div className="flex gap-3 flex-wrap max-w-xs">
              {moodsList.map(({ label, Icon, color }) => (
                ...
                >
                  <Icon size={18} className={selectedMood === label ? 'text-white' : color} />
                  {label}
                </button>
              ))}
            </div>
            ...
            <textarea
              rows="1"
              className="flex-1 p-2 border border-indigo-300 rounded-xl focus:outline-none focus:ring-4 focus:ring-indigo-400 resize-none placeholder-indigo-400 text-indigo-800 text-sm"
              placeholder="Note sur votre humeur du jour..."
              value={notes}
            ...
            <button
              type="submit"
              className="px-6 py-2 rounded-full bg-indigo-700 text-white font-bold text-sm hover:bg-indigo-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {/* Affichage humeur du jour si existante */}
      {humeurDuJour && (
