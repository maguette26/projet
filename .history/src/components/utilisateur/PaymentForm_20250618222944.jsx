...
const CheckoutForm = ({ clientSecret, onSuccess, onError, reservationId }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Confirmation du paiement via Stripe avec le clientSecret obtenu
      ...
      if (error) {
        ...
        // Après succès Stripe, on informe le backend que la réservation est payée
        await axios.post(
          `/api/reservations/payer/${reservationId}`,
          {},
...
const PaymentForm = ({ reservationId, onClose }) => {
  ...
  // Création du paiement côté backend (backend doit retourner un clientSecret Stripe ou une URL PayPal)
  ...
  useEffect(() => {
    ...
    if (paymentMethod === 'stripe') {
      createPayment()
        .then((data) => {
          // On récupère le clientSecret pour Stripe
  ...
  // Fermer le formulaire si paiement réussi
  useEffect(() => {
