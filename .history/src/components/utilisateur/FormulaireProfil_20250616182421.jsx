...
const FormulaireProfil = () => {
  const [profil, setProfil] = useState({
    ...
    photoUrl: '' // URL de la photo existante
  });
  const [photoFile, setPhotoFile] = useState(null); // fichier photo uploadé
  const [photoPreview, setPhotoPreview] = useState(null); // preview local
      ...
      try {
        ...
        if (data && data.email) {
          setProfil(prev => ({
            ...
            photoUrl: data.photoUrl || '', // adapter selon ta réponse API
            nouveauMotDePasse: '',
            confirmNouveauMotDePasse: ''
          }));
          setError(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Si ton API accepte FormData (pour envoyer la photo)
      const formData = new FormData();
      formData.append('nom', nom.trim());
      formData.append('prenom', prenom.trim());
      formData.append('telephone', telephone.trim());
      if (nouveauMotDePasse) formData.append('motDePasse', nouveauMotDePasse);
      if (photoFile) formData.append('photo', photoFile);

      await modifierProfil(formData);

      setSuccessMessage("Profil mis à jour avec succès !");
      setProfil(prev => ({ ...prev, nouveauMotDePasse: '', confirmNouveauMotDePasse: '' }));

      // Si la photo a changé, mettre à jour l’aperçu
      if (photoPreview) {
        setProfil(prev => ({ ...prev, photoUrl: photoPreview }));
        setPhotoFile(null);
    ...
    <div className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-lg mt-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-indigo-500 mb-3 shadow-lg">
            <img
              src={photoPreview || profil.photoUrl || '/default-profile.png'}
              alt="Photo de profil"
          ...
          <label
            htmlFor="photo"
            className="cursor-pointer bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-5 rounded-md shadow-md transition"
          >
            Choisir une photo
          </label>
          <input
        ...
        <fieldset className="border-t border-gray-300 pt-6">
          ...
          <div className="mb-4">
            <label htmlFor="nouveauMotDePasse" className="block text-sm font-semibold text-gray-700 mb-1">
          ...
          <div>
            <label htmlFor="confirmNouveauMotDePasse" className="block text-sm font-semibold text-gray-700 mb-1">
