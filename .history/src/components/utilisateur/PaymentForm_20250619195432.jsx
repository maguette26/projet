...
const PaymentForm = ({ reservationId, onClose }) => {
  ...
  const createPayment = async () => {
    try {
      ...
      console.error("❌ Veuillez attendre que le professionnel de santé confirme votre réservation :", error);
      throw error;
    }
  };
      ...
      createPayment()
        ...
        .catch(() => {
          if (isMounted) {
            setMessage({
              type: 'error',
              text: 'Veuillez attendre que le professionnel valide votre réservation avant de payer.',
            });
          }
        });
    }
  ...
  useEffect(() => {
    setIsWaitingBackend(message?.type === 'success' && paymentMethod === 'stripe');
  ...
  return (
    <div className="bg-white rounded p-6 w-full max-w-md shadow-lg relative max-h-screen overflow-y-auto" role="dialog" aria-modal="true">
      <button
        onClick={onClose}
