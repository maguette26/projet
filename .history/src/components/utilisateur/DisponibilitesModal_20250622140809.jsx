      ...
      <AnimatePresence>
        ...
        >
          <motion.div
            ...
            className="relative bg-white p-8 rounded-3xl max-w-5xl w-full max-h-[85vh] overflow-y-auto shadow-2xl border border-gray-200"
              ...
              <div className="space-y-8">
                {disponibilites.map((dispo, idx) => {
                  ...
                  return (
                    <div
                      key={`${dispo.date}-${idx}`}
                      className="bg-gray-50 border-l-4 border-blue-400 p-6 rounded-xl shadow-sm"
                    >
                      <div className="mb-3">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {sousCreneaux.map((heure, i) => {
                          ...
                          const dejaReserve = reservationsActives.some(
                            (resv) =>
                              ['PAYEE', 'EN_ATTENTE_PAIEMENT', 'EN_ATTENTE'].includes(resv.statut) &&
                              resv.heureConsultation === heure
                          );
                          ...
                          if (resvAPayer) {
                            ...
                            return (
                              <button
                                ...
                                disabled
                                className="px-4 py-2 rounded-full bg-gray-300 text-gray-600 text-sm font-medium cursor-not-allowed"
                              >
                                {label} (réservé)
                              </button>
                            );
