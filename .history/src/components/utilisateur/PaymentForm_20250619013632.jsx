...
const PaymentForm = ({ reservationId, onClose }) => {
  ...
  return (
    <div
      className="bg-white rounded p-6 w-full max-w-md shadow-lg relative max-h-[80vh] overflow-y-auto"
      role="dialog"
      aria-modal="true"
    >
      <button
      ...
      {paymentMethod === 'paypal' && (
        <button
          ...
          disabled={loadingPayment}
          className="px-4 py-2 bg-yellow-500 text-white rounded w-full disabled:opacity-50"
        >
          {loadingPayment ? 'Redirection PayPal...' : 'Payer avec PayPal'}
        </button>
      )}
    </div>
  );
