...
const ListeProfessionnels = () => {
  ...
  try {
    ...
    console.log("Professionnels bruts :", res.data); // ← pour vérifier le champ
    const valides = res.data.filter(p => p.valide === 'VALIDE'); // ← adapte le nom si besoin
    setProfessionnels(valides);
    setError('');

    const specs = Array.from(new Set(valides.map(p => p.specialite))).sort();
