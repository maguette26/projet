...
const Chatbot = () => {
  ...
  // 🔹 Ecouter changement login/logout
  useEffect(() => {
    const handleStorage = () => {
  ...
  // 🔹 Charger messages selon utilisateur
  useEffect(() => {
    const userId = localStorage.getItem("userId");

    if (!userId) {
      // ❌ Non connecté → message par défaut uniquement
      setMessages([
        {
          sender: "bot",
    ...
    // ✅ Connecté → charger son historique
 ...
 const sendMessage = async () => {
  if (!isLoaded) return; // 🔥 empêche bug premier message
  ...
  try {
    ...
    setMessages((prev) => [
      ...
      {
        sender: "bot",
        text: "Je rencontre un problème de connexion au serveur 😢",
        time: new Date().toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
