...
const checkReservationExists = async (professionnelId, dateConsultation) => {
  try {
    const res = await axios.get('/api/reservations/check-exists', {
      params: { professionnelId, dateConsultation },
      withCredentials: true,
    });
    return res.data.exists; // true ou false
  } catch (error) {
    console.error('Erreur lors de la vérification de réservation existante', error);
    toast.error('Impossible de vérifier la réservation existante.');
    return false; // par défaut autoriser la réservation en cas d'erreur réseau
  }
};
 const reserverCreneau = async (dispo, heureConsultation) => {
  ...
  // Vérifier si l'utilisateur a déjà une réservation à cette date avec ce pro
  const dejaReserve = await checkReservationExists(dispo.professionnel.id, dispo.date);
  if (dejaReserve) {
    toast.error("Vous avez déjà une réservation avec ce professionnel à cette date.");
    return;
  }
