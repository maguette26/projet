  ...
  return (
    <Layout>
      ...
      >
        ...
        >
          <Smile className="w-[10px] h-[10px] text-indigo-600 animate-pulse" style={{ filter: 'drop-shadow(0 0 1px #6366f1)' }} />
          <h1 className="text-[9px] font-extrabold text-indigo-900 leading-none tracking-tight select-none">
        ...
        <motion.p
          ...
          className="text-center text-gray-700 max-w-xs mb-[2px] px-1 text-[7px] font-semibold select-none"
        >
          Débloquez l'accès illimité à toutes nos ressources exclusives : exercices avancés, vidéos guidées, podcasts experts, et plus.
        </motion.p>

        <section className="bg-white rounded border border-indigo-300 p-1 max-w-xs w-full flex flex-col items-center">
          {Object.entries(plans).map(([key, plan]) => (
            <div key={key} className="mb-[2px] w-full px-1 border-b border-indigo-100 last:border-none">
              <p className="text-[8px] font-extrabold text-gray-900 leading-none tracking-tight select-none">
                {plan.label} : {plan.price.toFixed(2)} € / <span className="text-[6px] text-gray-500">{plan.duration}</span>
              </p>
              <p className="text-[5px] text-gray-600 font-semibold select-none">{plan.description}</p>
            </div>
          ))}
        </section>
      </main>
