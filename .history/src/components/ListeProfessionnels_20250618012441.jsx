...
const ListeProfessionnels = () => {
  ...
  // Nouveau state : créneaux réservés en local (par id de dispo + heure)
  ...
  const fetchDisponibilites = async (proId) => {
    try {
      ...
      setCreneauxReserves([]); // reset des créneaux réservés quand on change de pro
  ...
  const reserverCreneau = async (dispo, heureConsultation) => {
    ...
    const heureFormatee = `${heureConsultation}:00`; // ex: "12:00:00"
    ...
    try {
      ...
      // On ajoute ce créneau à la liste des réservés pour ne plus l'afficher
  ...
  // Fonction utilitaire pour savoir si un créneau est réservé
  const estReserve = (dispoId, heure) => {
    return creneauxReserves.some(
      (creneau) => creneau.dispoId === dispoId && creneau.heure === heure
    );
  };
                  ...
                  {disponibilites.map((dispo, idx) => (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {genererSousCreneaux(dispo)
                          .filter(heure => !estReserve(dispo.id, heure)) // filtrer les réservés
                          .map((heure, i) => (
                            <button
                              key={i}
