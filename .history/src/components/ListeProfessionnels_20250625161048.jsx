...
const ListeProfessionnels = () => {
  ...
  useEffect(() => {
    const filtered = professionnels.filter(pro =>
      ...
      (`${pro.nom} ${pro.prenom}`.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    setFilteredPros(filtered);
  }, [searchTerm, selectedSpecialite, professionnels]);
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 min-h-[80vh]">
        <h2 className="text-3xl font-bold text-center mb-6">👨‍⚕️ Découvrez nos professionnels</h2>
        <p className="text-center text-gray-600 mb-6">Prenez soin de votre santé mentale avec des experts à votre écoute.</p>
          ...
          {filteredPros.map((pro, index) => (
            ...
            >
              <div className="flex items-center gap-4 mb-4">
                <img
                  src={`https://ui-avatars.com/api/?name=${pro.prenom}+${pro.nom}&background=random&color=fff`}
                  alt={`${pro.nom} ${pro.prenom}`}
                  className="w-14 h-14 rounded-full object-cover border"
                />
                <div>
                  <p className="text-lg font-semibold">{pro.prenom} {pro.nom}</p>
                  <p className="text-sm text-gray-500">{pro.specialite}</p>
                </div>
              </div>
              <button
