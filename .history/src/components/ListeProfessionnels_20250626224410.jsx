...
const DevenirPremium = () => {
  ...
  return (
    <Layout>
      <main className="flex flex-col items-center justify-start min-h-[150vh] min-w-full pt-4 pb-4 bg-gradient-to-tr from-indigo-50 via-white to-indigo-50">
        <motion.div
        ...
        >
          ...
          />
          <h1 className="text-lg font-extrabold text-indigo-900 tracking-tight leading-none">
        ...
        <motion.p
          ...
          className="text-center text-gray-700 max-w-3xl mb-4 px-3 text-[11px] font-medium"
        >
        ...
        <motion.section
          ...
          className="bg-white rounded-lg shadow-sm border border-indigo-300 p-4 max-w-lg w-full flex flex-col items-center"
        >
          <h2 className="text-sm font-semibold text-indigo-700 mb-3 flex items-center gap-1">
            <CreditCard 
              className="w-4 h-4 text-indigo-700 animate-pulse" 
              style={{ filter: 'drop-shadow(0 0 1.5px #4338ca)' }}
            />{" "}
          ...
          <div className="flex justify-center space-x-2 mb-5">
            {Object.keys(plans).map((planKey) => (
              <button
                ...
                className={`px-2 py-0.5 rounded-full text-[9px] font-semibold transition duration-200 focus:outline-none focus:ring-1 focus:ring-indigo-500 ${
          ...
          <div className="mb-3 text-center">
            <p className="text-2xl font-extrabold text-gray-900 mb-0.5 tracking-tight">
              {currentPlan.price.toFixed(2)} € <span className="text-sm text-gray-500">/ {currentPlan.duration}</span>
            </p>
            <p className="text-[10px] text-gray-600 max-w-lg mx-auto font-medium">{currentPlan.description}</p>
          </div>

          {loading ? (
            <p className="text-indigo-600 font-semibold text-xs animate-pulse">Chargement du bouton de paiement...</p>
          ) : error ? (
            <div className="bg-red-100 border border-red-400 text-red-700 px-2 py-1 rounded mb-3 text-xs max-w-md mx-auto">
              {error}
            </div>
          ) : success ? (
            <div className="bg-green-100 border border-green-400 text-green-700 px-2 py-1 rounded mb-3 text-xs max-w-md mx-auto">
