            ...
            {currentRole === "UTILISATEUR" && (
              <div className="relative" ref={menuRef}>
                <button
                  ...
                  className="flex items-center gap-1 text-indigo-600  transition font-medium"
                >
                  <User className="w-4 h-4" /> Mon Espace
                  <ChevronDown className={`w-3 h-3 transition-transform ${userMenuOpen ? 'rotate-180' : ''}`} />
                </button>

                {/* Menu visible / invisible avec Tailwind */}
                <div className={`absolute right-0 mt-2 w-48 bg-white border border-gray-200 shadow-md rounded-md flex flex-col z-50 transition-all duration-200 transform ${userMenuOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
                  <Link
