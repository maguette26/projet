...
import { BookOpen, LogOut, User, Brain, Info, MessageCircle, UserCog, Crown } from 'lucide-react';
...
const Header = () => {
  ...
  // Gestion du changement dans le select (redirige selon choix)
    ...
    <header className="bg-white/80 backdrop-blur-md sticky top-0 z-50 shadow-md h-[70px]">
      <div className="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center h-full">
        ...
        <nav className="flex items-center gap-6 text-sm font-semibold">
          ...
          {/* Si pas connecté, affiche un select */}
          {!currentRole && (
            <select
              onChange={handleSelectChange}
              defaultValue=""
              className="text-indigo-600 bg-transparent border border-indigo-600 rounded px-2 py-1 cursor-pointer"
            >
