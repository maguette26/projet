      ...
      <header className="bg-white/95 backdrop-blur-md fixed top-0 w-full z-50 shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center h-[70px]">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link to="/chatbot" className={baseLinkCl}>ass<Bot className="w-4 h-4" /> PsyBotAI</Link>
            ...
            {currentRole === "UTILISATEUR" && (
              <div className="relative" ref={menuRef}>
                <button
                  ...
                  className="flex items-center gap-1 text-indigo-600 hover:text-indigo-800 transition font-medium focus:outline-none"
                >
                  <User className="w-4 h-4" /> Mon Espace
                  <ChevronDown className={`w-3 h-3 transition-transform ${userMenuOpen ? 'rotate-180' : ''}`} />
                </button>

                {/* Menu avec animation Tailwind */}
                <div className={`absolute right-0 mt-2 w-48 bg-white border border-gray-200 shadow-lg rounded-md flex flex-col z-50 transition-all duration-200 transform origin-top-right
