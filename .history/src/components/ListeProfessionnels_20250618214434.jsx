...
const ListeProfessionnels = () => {
  ...
  // ** Nouvel état pour les réservations non payées **
  const [reservationsNonPayees, setReservationsNonPayees] = useState([]);
  ...
  useEffect(() => {
    ...
    // ** Nouvelle fonction pour charger les réservations non payées **
    const fetchReservationsNonPayees = async () => {
      try {
        const res = await axios.get('/api/reservations/non-payees', { withCredentials: true });
        setReservationsNonPayees(res.data);
      } catch (err) {
        console.error('Erreur chargement réservations non payées', err);
      }
    };

    fetchProfessionnels();
    fetchReservationsNonPayees();
  }, []);
  ...
  const reserverCreneau = async (dispo, heureConsultation) => {
    ...
    try {
      ...
      // ** Mettre à jour la liste des réservations non payées en la rechargant **
      const resNonPayees = await axios.get('/api/reservations/non-payees', { withCredentials: true });
      setReservationsNonPayees(resNonPayees.data);
    } catch (error) {
      if (error.response) {
  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 min-h-[80vh]">
        ...
        {/* --- Affichage des réservations non payées --- */}
        {reservationsNonPayees.length > 0 && (
          <section className="mb-10">
            <h3 className="text-2xl font-bold mb-4 text-center">Réservations en attente de paiement</h3>
            <ul className="space-y-4 max-w-md mx-auto">
              {reservationsNonPayees.map((resv) => (
                <li key={resv.id} className="border p-4 rounded-md shadow-sm flex justify-between items-center">
                  <div>
                    <p>
                      {new Date(resv.heureConsultation).toLocaleDateString()} à {new Date(resv.heureConsultation).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                    </p>
                    <p className="text-gray-600">Professionnel : {resv.disponibilite?.professionnel?.prenom} {resv.disponibilite?.professionnel?.nom}</p>
                  </div>
                  <button
                    onClick={() => setReservationIdPourPaiement(resv.id)}
        ...
        {reservationIdPourPaiement && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <PaymentForm
              ...
              onClose={() => {
                setReservationIdPourPaiement(null);
                // Met à jour la liste des réservations non payées après paiement (à adapter si tu as un event ou un callback)
                axios.get('/api/reservations/non-payees', { withCredentials: true }).then(res => setReservationsNonPayees(res.data));
              }}
            />
          </div>
        )}
