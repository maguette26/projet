...
const ListeProfessionnels = () => {
  ...
  // Charger police Poppins pour toute la page
  useEffect(() => {
  ...
  return (
    <>
      ...
      <main
        className="max-w-6xl mx-auto px-6 min-h-[80vh]"
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.section
              ...
              animate="animate"
              exit="exit"
              className="relative w-full h-screen text-center px-8 py-20 bg-gradient-to-tr from-indigo-50 via-indigo-100 to-white overflow-hidden flex flex-col justify-center"
            >
              {/* Décors flous animés */}
              <div
                aria-hidden="true"
              ...
              <h1 className="relative text-5xl font-extrabold text-indigo-900 mb-6 leading-tight tracking-wide drop-shadow-md">
                Bienvenue sur <span className="text-indigo-600">PsyConnect</span>
              </h1>
              <h2 className="relative text-xl italic text-indigo-700 mb-12 max-w-xl mx-auto drop-shadow-sm">
              ...
              <button
                onClick={() => setStep(2)}
                className="mt-16 relative inline-flex items-center justify-center gap-4 bg-indigo-700 hover:bg-indigo-800 text-white font-semibold rounded-full px-12 py-4 shadow-lg hover:shadow-indigo-600 transition-all duration-300 active:scale-95"
                ...
                <div className="flex flex-wrap gap-4 items-center">
                  <div className="flex items-center gap-3 bg-indigo-50 rounded-md px-4 py-2 border border-indigo-200 shadow-sm">
                    ...
                    <select
                      ...
                      className="appearance-none bg-white border border-indigo-300 rounded-md px-3 py-2 text-indigo-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium cursor-pointer"
                ...
                {professionnelsFiltres.length === 0 ? (
                  ...
                  professionnelsFiltres.map((pro) => (
                    <motion.div
                      ...
                      className="bg-white border border-indigo-100 rounded-2xl shadow-md hover:shadow-xl transition-all p-6 flex flex-col justify-between relative"
                    >
                      {/* Badge spécialité */}
                      <span className="absolute top-4 right-4 bg-indigo-100 text-indigo-700 text-xs font-semibold px-3 py-1 rounded-full select-none">
                        {pro.specialite}
                      </span>

                      <div className="flex items-center gap-3 mb-4">
                        {iconSpecialite(pro.specialite)}
                        <h3 className="text-xl font-bold text-gray-900 select-text">
                          Dr {pro.prenom} {pro.nom}
                        </h3>
                      </div>

                      <p className="text-gray-600 text-sm mb-6 leading-relaxed min-h-[3rem]">
                        {pro.presentation || "Ce professionnel n’a pas encore ajouté de description."}
                      </p>

                      <button
                        onClick={() => fetchDisponibilites(pro.id)}
                        className="mt-auto px-5 py-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 flex items-center justify-center gap-3 font-semibold transition-shadow shadow-md active:scale-95"
                      >
