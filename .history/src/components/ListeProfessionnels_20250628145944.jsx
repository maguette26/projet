...
useEffect(() => {
  const fetchProfessionnels = async () => {
    try {
      ...
      // spécialités extraites des données
      const specsFromData = Array.from(new Set(res.data.map(p => p.specialite).filter(Boolean)));

      // spécialités par défaut à toujours afficher
      const defaultSpecs = ['Psychiatre', 'Psychologue'];

      // fusion et suppression des doublons
      const uniqueSpecs = Array.from(new Set([...defaultSpecs, ...specsFromData])).sort();
