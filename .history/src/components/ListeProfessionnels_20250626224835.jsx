...
const DevenirPremium = () => {
  ...
  return (
    <Layout>
      <main className="flex flex-col items-center justify-start min-h-screen min-w-full pt-2 pb-1 bg-gradient-to-tr from-indigo-50 via-white to-indigo-50">
        <motion.div
        ...
        >
          <Smile className="w-3 h-3 text-indigo-600 animate-pulse" style={{ filter: 'drop-shadow(0 0 1px #6366f1)' }} />
          <h1 className="text-[10px] font-extrabold text-indigo-900 leading-none tracking-tight">
        ...
        <motion.p
          ...
          className="text-center text-gray-700 max-w-3xl mb-0.5 px-1 text-[6.5px] font-semibold"
        >
        ...
        <motion.section
          ...
          className="bg-white rounded-sm shadow-sm border border-indigo-300 p-1.5 max-w-xs w-full flex flex-col items-center"
        >
          <h2 className="text-[8.5px] font-semibold text-indigo-700 mb-0.5 flex items-center gap-1">
            <CreditCard className="w-2.5 h-2.5 text-indigo-700 animate-pulse" style={{ filter: 'drop-shadow(0 0 1px #4338ca)' }} />{' '}
          ...
          <div className="flex justify-center space-x-0.5 mb-2">
            {Object.keys(plans).map((planKey) => (
              <button
                ...
                className={`px-1 py-0.5 rounded-full text-[6px] font-semibold transition duration-100 focus:outline-none focus:ring-1 focus:ring-indigo-500 ${
          ...
          <div className="mb-0.5 text-center">
            <p className="text-base font-extrabold text-gray-900 mb-0 leading-none tracking-tight">
              {currentPlan.price.toFixed(2)} € <span className="text-[8px] text-gray-500">/ {currentPlan.duration}</span>
            </p>
            <p className="text-[6.5px] text-gray-600 max-w-lg mx-auto font-semibold">{currentPlan.description}</p>
          </div>

          {loading ? (
            <p className="text-indigo-600 font-semibold text-[9px] animate-pulse">Chargement du bouton de paiement...</p>
          ) : error ? (
            <div className="bg-red-100 border border-red-400 text-red-700 px-1 py-1 rounded mb-1 text-[7px] max-w-sm mx-auto">
              {error}
            </div>
          ) : success ? (
            <div className="bg-green-100 border border-green-400 text-green-700 px-1 py-1 rounded mb-1 text-[7px] max-w-sm mx-auto">
