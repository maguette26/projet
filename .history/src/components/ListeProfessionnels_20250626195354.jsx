...
const ListeProfessionnels = () => {
 ...
 const reserverCreneau = async (dispo, heureConsultation) => {
  ...
  // Prépare les données pour la réservation
  ...
  try {
    // 1. Vérifie d'abord si l'utilisateur a déjà une réservation ce jour-là pour ce pro
    const responseCheck = await axios.get('/api/reservations/check-exists', {
      params: {
        professionnelId: dispo.professionnelId,  // Il faut que tu passes l'id du professionnel ici
        dateConsultation: dispo.date,            // Date de la dispo (= date consultation)
      },console.log("proId:", dispo.professionnelId, "date:", dispo.date);
const responseCheck = await axios.get('/api/reservations/check-exists', {
  params: {
    professionnelId: dispo.professionnelId,
    dateConsultation: dispo.date,
  },
  withCredentials: true,
    ...
    if (responseCheck.data.exists) {
      toast.error("❌ Vous avez déjà une réservation ce jour-là avec ce professionnel.");
      return;  // On stoppe la réservation
    }

    // 2. Si pas de doublon, on fait la réservation
    ...
    if (error.response) {
      // Récupération améliorée du message d'erreur
      const data = error.response.data;
      let message = "Réservation échouée";
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.section
              ...
              animate="animate"
              exit="exit"
              className="relative max-w-4xl mx-auto text-center px-8 py-20 rounded-3xl bg-gradient-to-tr from-indigo-50 via-indigo-100 to-white shadow-2xl overflow-hidden"
            >
              {/* Décors flous animés */}
              <div
                aria-hidden="true"
