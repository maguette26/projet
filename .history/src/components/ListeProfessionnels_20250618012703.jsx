...
const ListeProfessionnels = () => {
  ...
  // On garde une liste des créneaux réservés en local pour les cacher
  ...
  const fetchDisponibilites = async (proId) => {
    try {
      ...
      setCreneauxReserves([]); // Reset les réservations locales quand on change de pro
  ...
  const reserverCreneau = async (dispo, heureConsultation) => {
    ...
    try {
      ...
      // Ajout du créneau réservé dans le state local pour le cacher
  ...
  // Vérifie si un créneau est déjà réservé (pour le cacher)
  const estReserve = (dispoId, heure) => {
    return creneauxReserves.some(cr => cr.dispoId === dispoId && cr.heure === heure);
  };
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {genererSousCreneaux(dispo)
                          .filter(heure => !estReserve(dispo.id, heure))
                          .map((heure, i) => (
                            <button
                              key={i}
