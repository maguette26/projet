...
const ListeProfessionnels = () => {
  ...
  // Stocke les créneaux déjà réservés localement
  ...
  const fetchDisponibilites = async (proId) => {
    try {
      ...
      setCreneauxReserves([]); // Reset les créneaux réservés quand on change de pro
  ...
  const reserverCreneau = async (dispo, heureConsultation) => {
    ...
    const heureReservation = `${heureConsultation}:00`; // "12:00:00"
    ...
    try {
      ...
      // Ajoute le créneau à la liste des réservés pour le cacher
      setCreneauxReserves(prev => [...prev, { dispoId: dispo.id, heure: heureConsultation }]);
    } catch (error) {
      if (error.response) {
  ...
  // Fonction d'annulation qui fait réapparaître le créneau
  const annulerReservation = (dispoId, heure) => {
    // Ici, tu peux aussi faire appel à une API pour annuler côté serveur si tu veux
    setCreneauxReserves(prev => prev.filter(cr => !(cr.dispoId === dispoId && cr.heure === heure)));
    toast.info(`Réservation à ${heure} annulée.`);
  };
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {genererSousCreneaux(dispo).map((heure, i) => {
                          const estReserve = creneauxReserves.some(
                            (cr) => cr.dispoId === dispo.id && cr.heure === heure
                          );
                          ...
                          if (estReserve) {
                            return (
                              <button
                                key={i}
                                onClick={() => annulerReservation(dispo.id, heure)}
