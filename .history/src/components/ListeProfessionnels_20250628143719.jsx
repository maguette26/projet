  ...
  useEffect(() => {
    const fetchProfessionnels = async () => {
      try {
        ...
        // Ajout manuel de "Psychiatre" si pas déjà dans la liste des spécialités
        const specsSet = new Set(res.data.map(p => p.specialite));
        if (!specsSet.has('Psychiatre')) specsSet.add('Psychiatre');
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.section
              ...
              animate="animate"
              exit="exit"
              className="relative w-full min-h-[65vh] text-center px-8 py-16 bg-gradient-to-tr from-indigo-50 via-indigo-100 to-white overflow-hidden flex flex-col justify-center"
            >
              {/* Décors flous animés */}
              <div
                aria-hidden="true"
              ...
              <button
                onClick={() => setStep(2)}
                className="mt-14 relative inline-flex items-center justify-center gap-3 bg-indigo-700 hover:bg-indigo-800 text-white font-semibold rounded-full px-10 py-3 shadow-lg hover:shadow-indigo-600 transition-all duration-300 active:scale-95"
