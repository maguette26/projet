...
const DevenirPremium = () => {
  ...
  return (
    <Layout>
      <main className="flex flex-col items-center justify-start min-h-[150vh] min-w-full pt-6 pb-6 bg-gradient-to-tr from-indigo-50 via-white to-indigo-50">
        <motion.div
        ...
        >
          ...
          />
          <h1 className="text-xl md:text-2xl font-extrabold text-indigo-900 tracking-tight leading-tight">
        ...
        <motion.p
          ...
          className="text-center text-gray-700 max-w-3xl mb-6 px-3 text-xs md:text-sm font-medium"
        >
        ...
        <motion.section
          ...
          className="bg-white rounded-xl shadow-md border border-indigo-300 p-6 max-w-xl w-full flex flex-col items-center"
        >
          <h2 className="text-base md:text-lg font-semibold text-indigo-700 mb-4 flex items-center gap-1">
            <CreditCard 
              className="w-4 h-4 text-indigo-700 animate-pulse" 
              style={{ filter: 'drop-shadow(0 0 2px #4338ca)' }}
            />{" "}
          ...
          <div className="flex justify-center space-x-3 mb-6">
            {Object.keys(plans).map((planKey) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-[10px] md:text-xs font-semibold transition duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
          ...
          <div className="mb-4 text-center">
            <p className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-1 tracking-tight">
              {currentPlan.price.toFixed(2)} € <span className="text-base md:text-lg text-gray-500">/ {currentPlan.duration}</span>
            </p>
            <p className="text-xs text-gray-600 max-w-lg mx-auto font-medium">{currentPlan.description}</p>
          </div>

          {loading ? (
            <p className="text-indigo-600 font-semibold text-xs animate-pulse">Chargement du bouton de paiement...</p>
          ) : error ? (
            <div className="bg-red-100 border border-red-400 text-red-700 px-3 py-1.5 rounded mb-4 text-xs max-w-md mx-auto">
              {error}
            </div>
          ) : success ? (
            <div className="bg-green-100 border border-green-400 text-green-700 px-3 py-1.5 rounded mb-4 text-xs max-w-md mx-auto">
