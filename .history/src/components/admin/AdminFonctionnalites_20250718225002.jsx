...
function AdminFonctionnalites() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!nom.trim()) {
      toast.error("Le nom de la fonctionnalité est obligatoire.");
      return;
    }
  ...
  const handleImport = async () => {
    if (!excelFile) {
      toast.error("Veuillez choisir un fichier Excel.");
      return;
    }
    ...
    try {
      ...
      toast.success(res.data || "Importation réussie !");
      setExcelFile(null);
      fetchFonctionnalites();
    } catch (err) {
  ...
  <button 
    ...
    className="text-indigo-600 hover:text-indigo-900 focus:outline-none focus:ring-0"
  >
    <FontAwesomeIcon icon={faPencilAlt} />
  </button>
  <button 
    onClick={() => handleDelete(f.id)} 
    className="text-red-600 hover:text-red-800 focus:outline-none focus:ring-0"
  >
