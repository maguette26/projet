...
const GestionProfessionnels = () => {
  ...
  const handleValidation = async (id, valide) => {
    const confirmation = window.confirm(
      valide
        ? "Voulez-vous valider ce professionnel ?"
        : "Voulez-vous refuser ou retirer ce professionnel ?"
    );
    ...
    <div className="p-6">
      ...
      {filteredProfessionnels.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-300 shadow">
          <table className="min-w-full bg-gray-50 divide-y divide-black">
            <thead className="bg-indigo-600 text-white divide-x divide-black">
              <tr>
            ...
            <tbody className="divide-y divide-black">
              {filteredProfessionnels.map(pro => (
                <tr key={pro.id} className="divide-x divide-black">
                  <td className="px-4 py-3 text-sm font-medium text-black">{pro.nom}</td>
                  <td className="px-4 py-3 text-sm text-black">{pro.prenom}</td>
                  <td className="px-4 py-3 text-sm text-black">{pro.email}</td>
                  <td className="px-4 py-3 text-sm text-black">{pro.telephone || 'N/A'}</td>
                  <td className="px-4 py-3 text-sm text-black capitalize">{pro.specialite || 'N/A'}</td>
