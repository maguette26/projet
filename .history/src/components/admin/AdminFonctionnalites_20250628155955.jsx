...
// Import de SheetJS et FileSaver pour l'export Excel
...
function AdminFonctionnalites() {
  ...
  // Import Excel (garde ton code ici si tu veux)
  ...
  // Nouvelle fonction pour exporter les fonctionnalités en Excel
  const handleExport = () => {
    if (!fonctionnalites || fonctionnalites.length === 0) {
      setError("Aucune fonctionnalité à exporter.");
      return;
    }

    // Préparer les données (exclure les champs non pertinents si besoin)
    const exportData = fonctionnalites.map(({ id, nom, type, description, lienFichier, categorie, statut, premium }) => ({
      ID: id,
      Nom: nom,
      Type: type,
      Description: description,
      'Lien Fichier': lienFichier || '',
      Catégorie: categorie || '',
      Statut: statut ? 'Actif' : 'Inactif',
      Premium: premium ? 'Oui' : 'Non',
    }));
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Fonctionnalites");
    ...
    saveAs(blob, 'fonctionnalites_export.xlsx');

    setSuccessMessage("Exportation Excel réussie !");
  };
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="bg-white p-4 rounded shadow mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div>
          <h2 className="text-lg font-semibold mb-2 flex items-center gap-2">
            <FontAwesomeIcon icon={faFileExport} /> Exporter vers Excel
          </h2>
          <button
