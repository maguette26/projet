          ...
          <table className="min-w-full bg-gray-50">
            <thead className="bg-primary-600 text-white">
              <tr>
                {['Nom Prénom', 'Email', 'Téléphone', 'Spécialité', 'Statut', 'Document', 'Actions'].map((title, idx) => (
                  <th
                    key={idx}
                    className="px-6 py-3 text-xs font-semibold text-left uppercase tracking-wider"
                  >
                    {title}
                  </th>
                ))}
            ...
            <tbody>
              {filteredProfessionnels.map(pro => (
                <tr
                  key={pro.id}
                  className="bg-white even:bg-gray-100 hover:bg-gray-200 transition-colors duration-200"
                >
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">{pro.telephone || 'N/A'}</td>
                  <td className="px-6 py-4 text-sm text-gray-700 capitalize">{pro.specialite || 'N/A'}</td>
